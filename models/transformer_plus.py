...
class UniversalEncoder(nn.Module):
    ...
    def __init__(self, dim = 256, num_head=4, num_layers=4, *args, **kwargs):
...
class CrossFusionEncoder(nn.Module):
    ...
    def __init__(self, embed_dim=256, num_heads=4, num_layers=1, **kwargs):
        """
        :param kwargs:
        """
        super(CrossFusionEncoder, self).__init__(**kwargs, **kwargs)
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.num_layers = num_layers

        # Text
        self.transformer_encoder_block_text = nn.TransformerEncoderLayer(
            d_model=self.embed_dim,
            nhead=self.num_heads,
            batch_first=True
        )
        self.transformer_encoder_text = nn.TransformerEncoder(
            self.transformer_encoder_block_text,
            num_layers=self.num_layers
        )

        self.attention_text = nn.MultiheadAttention(self.embed_dim, self.num_heads, batch_first=True)

        # Image
        self.transformer_encoder_block_image = nn.TransformerEncoderLayer(
            d_model=self.embed_dim,
            nhead=self.num_heads,
            batch_first=True
        )
        self.transformer_encoder_image = nn.TransformerEncoder(
            self.transformer_encoder_block_image,
            num_layers=self.num_layers
        )

        self.attention_image = nn.MultiheadAttention(self.embed_dim, self.num_heads, batch_first=True)
    ...
    def forward(self, inputs):
        ...
        cross_attention_image, _ = self.attention_image(images, texts, texts)
        image_features = self.transformer_encoder_image(images + cross_attention_image)

        cross_attention_text, _ = self.attention_text(texts, images, images)
        text_features = self.transformer_encoder_image(texts + cross_attention_text)
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.cross_fusion_encoder = CrossFusionEncoder()
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            # Fusion
            src, text_memory_resized = self.cross_fusion_encoder([src, text_memory_resized])



            # Universal Encoder
            src = self.universal_encoder(src, "image")
            text_memory_resized = self.universal_encoder(text_memory_resized, "text")
